import {
  MapPin, Briefcase, ChevronRight, Search, X,
  AlertCircle, CheckCircle, XCircle, Clock, User, ArrowLeft,
} from "lucide-react";
...
const LowonganPublikPage = () => {
  ...
  const getAllLowonganPublik = async (page = 1) => {
    try {
      setLoading(true);
      const response = await api.get(`/lowongan-kerja?page=${page}`);
  ...
  const getStatusConfig = (status) => {
    const configs = {
      pending: {
        ...
        icon: Clock, label: "Pending", badge: "bg-yellow-100",
        title: "Lamaran Sedang Diproses",
        description: "Lamaran Anda sedang dalam proses review oleh tim HRD",
      },
      diterima: {
        bg: "bg-green-50", border: "border-green-200", text: "text-green-800",
        icon: CheckCircle, label: "Diterima", badge: "bg-green-100",
        title: "Selamat! Lamaran Diterima",
        description: "Lamaran Anda telah diterima. Tim HRD akan menghubungi Anda segera",
      },
      ditolak: {
        bg: "bg-red-50", border: "border-red-200", text: "text-red-800",
        icon: XCircle, label: "Ditolak", badge: "bg-red-100",
        title: "Lamaran Belum Berhasil",
        description: "Mohon maaf, lamaran Anda belum dapat kami terima saat ini",
      },
    };
  ...
  const handleCekStatus = async (e) => {
    ...
    setErrorCekStatus("");
    try {
      const response = await api.post("/rekruitmen/cek", { token: tokenInput });
      ...
      return (
        <div className="bg-white rounded-lg max-w-md w-full p-6 relative">
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-600">Masukkan kode lamaran Anda untuk melihat status terkini</p>
          </div>
          <form onSubmit={handleCekStatus} className="space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              <div className="flex gap-2">
                ...
                <div>
                  <p className="text-xs text-blue-900 font-medium mb-1">Belum punya kode lamaran?</p>
                  <p className="text-xs text-blue-700">Kode diberikan setelah Anda mengirim lamaran melalui form aplikasi</p>
                </div>
              </div>
            </div>
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={handleCloseModal} disabled={loadingCekStatus} className="flex-1 px-4 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50">
                Batal
              </button>
              <button type="submit" disabled={loadingCekStatus || !tokenInput.trim()} className="flex-1 px-4 py-3 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 transition-colors disabled:bg-cyan-400 disabled:cursor-not-allowed font-medium flex items-center justify-center gap-2">
                {loadingCekStatus ? (
                  <><ClipLoader color="#ffffff" size={16} /><span>Mencari...</span></>
                ) : (
    ...
    return (
      <div className="bg-white rounded-lg max-w-lg w-full relative max-h-[85vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between z-10 rounded-t-lg">
          <button onClick={handleBackToSearch} className="flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors">
            <ArrowLeft size={20} />
        ...
        <div className="p-6 space-y-6">
          <div className={`rounded-lg border-2 p-6 ${statusConfig.bg} ${statusConfig.border}`}>
            <div className="flex items-start gap-4">
              <div className={`p-3 rounded-full ${statusConfig.badge}`}>
                <StatusIcon className={statusConfig.text} size={32} />
              </div>
              <div className="flex-1">
                <h2 className={`text-2xl font-bold mb-2 ${statusConfig.text}`}>{statusConfig.title}</h2>
                <p className={`text-sm ${statusConfig.text}`}>{statusConfig.description}</p>
              </div>
            </div>
          </div>
          <div className="bg-gradient-to-r from-cyan-50 to-blue-50 border border-cyan-200 rounded-lg p-4">
            <p className="text-xs text-gray-600 mb-1">Kode Lamaran Anda</p>
            <p className="text-lg font-bold text-cyan-700 break-all">{lamaran.token_pendaftaran}</p>
          </div>
          <div className="bg-white border border-gray-200 rounded-lg p-5">
            ...
            <div className="space-y-3 pl-7">
              <div>
                ...
                <p className="font-medium text-gray-900">{lamaran.nama_lengkap}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Status Lamaran</p>
                <span className={`inline-flex items-center gap-1 px-3 py-1 text-xs font-semibold rounded-full ${statusConfig.badge} ${statusConfig.text}`}>
                  <StatusIcon size={14} />
                  {statusConfig.label}
                </span>
              </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-xs text-blue-900 font-medium mb-2">💡 Informasi Penting</p>
            <ul className="text-xs text-blue-700 space-y-1 pl-4 list-disc">
              <li>Simpan kode lamaran Anda dengan baik</li>
              <li>Tim HRD akan menghubungi Anda melalui nomor yang terdaftar</li>
              <li>Pantau status lamaran Anda secara berkala</li>
            </ul>
          </div>
          <div className="pt-4 border-t">
            <button onClick={handleCloseModal} className="w-full px-4 py-3 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 transition-colors font-medium">
              Tutup
            </button>
          </div>
        </div>
  ...
  return (
    <>
      <Navbar />
      {showCekStatusModal && (
        <div onClick={handleBackdropClick} className="fixed inset-0 bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          {renderModalContent()}
        </div>
      )}
      ...
      <div className="min-h-screen bg-gray-50 pt-20">
        <div className="bg-blue-600 text-white py-16 pb-14">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 items-center text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-12">Lowongan Kerja</h1>
            <p className="text-xl md:text-2xl text-white mb-8">
              Bergabunglah dengan Tim Kami dan Kembangkan Karir Anda di Perusahaan HR Terbaik
            </p>
            <button
              ...
              {lowonganData.map((lowongan) => (
                <div key={lowongan.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 overflow-hidden border border-gray-200">
                  <div className="p-6">
                    ...
                    <p className="text-gray-700 text-sm mb-4 line-clamp-2">{lowongan.catatan}</p>
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm text-gray-600">Gaji:</span>
                      <p className="text-green-600 font-semibold text-right">{lowongan.range_gaji}</p>
                    </div>
                    <div className="flex items-center justify-between mb-6">
                      <span className="text-sm text-gray-600">Deadline</span>
                      <span className="text-sm text-gray-900">{formatDate(lowongan.deadline_lowongan)}</span>
                    </div>
                    <button
                      onClick={() => navigate(`/lowongan-publik/${lowongan.id}`)}
